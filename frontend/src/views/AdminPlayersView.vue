...
import { Search, ArrowUpDown, ArrowUp, ArrowDown, Database, X, Plus, Trash2, ChevronDown, ChevronUp, Check, Pencil, Filter, TrendingUp, TrendingDown, BarChart3, Hash } from 'lucide-vue-next'
...
<template>
  <div class="admin-players-view">
    ...
    <div class="kpi-grid" v-if="allPlayers.length">
      <div class="kpi-card kpi-highest" @click="highestEloPlayer && openPlayerModal(highestEloPlayer, highestElo)" :class="{ clickable: !!highestEloPlayer }">
      ...
      <div class="kpi-card kpi-lowest" @click="lowestEloPlayer && openPlayerModal(lowestEloPlayer, lowestElo)" :class="{ clickable: !!lowestEloPlayer }">
        <div class="kpi-icon-wrap">
        ...
        <table class="players-table">
          ...
          <tbody>
            <tr v-for="(player, index) in players" :key="`${player.name}-${player.latest_elo}`">
              ...
              <td class="col-name player-cell player-clickable" @click="openPlayerModal(player.name, player.latest_elo)">{{ player.name }}</td>
