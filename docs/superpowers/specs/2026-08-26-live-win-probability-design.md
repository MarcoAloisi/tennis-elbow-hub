...
class LiveMatchState:
    ...
    sets_won: tuple[int, int]                     # completed sets only (all but the last entry of `sets`) — winner per entry decided by comparing its two game counts
    current_set_games: tuple[int, int]            # numeric games from the last (in-progress) entry of `sets`; (0, 0) if `sets` is empty
...
`current_points_numeric`, `sets_won`, and `current_set_games` exist so `win_probability.py` never touches raw score tokens directly — `score_parser.py` stays the single owner of all score-token interpretation (point-table lookups, "who won this completed set" comparisons, tiebreak's raw-int parsing). Without these fields, the probability module would need its own private string-parsing logic — the exact duplication the score_parser split is meant to prevent. `win_probability.py`'s inputs are plain integers only.
...
- `implied_game_win_rate(s: float, games_per_set: int) -> float` / `implied_set_win_rate(p0: float, sets_to_win: int) -> float` — binary-search inverses of the above (both are monotonic and continuous in their rate parameter, so the inversion is well-defined). `implied_game_win_rate` needs `games_per_set` because `game_to_set_prob` does. Computed **once per match**, cached alongside `P0` (see Caching) — not re-solved every poll tick.
