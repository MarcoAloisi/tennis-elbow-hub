...
Private helper: `ELO_BAND` + sort/split + “pick cluster for E”, used by `get_player_clusters_async` and the details `elo` branch. **Do not** extract a new full-table appearance walker that replaces the details loop — that would silently change profile stats. List clusters use the same name parsing as `get_all_players_async`; details clusters only filter matches that loop already collected (the details loop already requires `" vs "` — leave that difference).
...
- `LiveScoresView.vue`: `useAuthStore()`. Guest vs logged-in must wait for `!auth.loading` — same as `App.vue` auth buttons (`loading` starts `true`). While auth is hydrating, a name click is a no-op (do not flash the signup prompt for a logged-in user). Then: guest → prompt, no fetch; logged-in → `<PlayerDetailsModal>`.
